...
6. [Store Layout Architecture](#store-layout-architecture)
7. [API Client & Interceptors](#api-client--interceptors)
8. [Role-Based Access Control](#role-based-access-control)
   - [System Roles](#system-roles)
   - [Organization Roles](#organization-roles)
   - [Enforcement Layers](#enforcement-layers)
9. [State Management Philosophy](#state-management-philosophy)
...
The Store Layout feature provides a full interactive grid editor for designing store floor plans. It is the most complex feature in the application, with its own layered architecture.

### Layer Overview

```
Types (shared/types/store.ts)
  └─ Constants (colors.ts, fixtures.ts)       ← Color/fixture definitions
       └─ Utils (grid.ts)                     ← Grid math helpers
            └─ API (storeApi.ts)              ← REST endpoint wrappers
                 └─ Hook (useLayoutEditor.ts) ← Editor state machine
                      └─ Components           ← Presentational UI
                           └─ Page            ← Orchestrator
```

### Grid Editor State Machine (`useLayoutEditor`)

The `useLayoutEditor` hook centralizes all editor interaction state (~15 state variables) into a single hook, preventing the page component from becoming a god component. It manages:

| State Group | Variables | Purpose |
| ----------- | --------- | ------- |
| **Placement** | `placementMode`, `freeformCells`, `pendingCells`, `pendingIsFreeform` | Track which tool is active and what cells the user is painting |
| **Selection** | `selectedItemId`, `selectedItemType` | Track which zone/fixture is highlighted in the grid and panel |
| **Editing** | `editingId`, `editingType`, `editingCells` | Track in-progress shape edits on existing items |
| **Modals** | `showZoneNameModal`, `showZoneDetailModal`, `showFixtureNameModal`, `showFixtureDetailModal` | Control which modal is open |

The hook also provides derived values (`editingName`, `editingColor`, `hintText`, `selectedZone`, `selectedFixture`) via `useMemo`, and reset helpers (`resetAfterCreate`, `resetAfterDelete`, `closeDetailAfterUpdate`) that ensure consistent state cleanup after CRUD operations.

### Grid Interaction Modes

The `LayoutGrid` component supports three interaction modes:

| Mode | Trigger | Behavior |
| ---- | ------- | -------- |
| **View** | Default (no placement active) | Click to select a zone/fixture. Double-click to open its detail modal. |
| **Placement** | User selects "Inventory Zone" or "Wall / Fixture" from the toolbar | Mouse-down and drag to paint cells. Cells toggle on/off. A banner shows the cell count with Done/Cancel buttons. |
| **Edit Shape** | "Edit Shape" button inside a detail modal | Similar to placement, but modifying the cells of an existing zone/fixture. Save/Cancel buttons in a banner. |

### Cell Rendering Performance

The grid pre-computes a `Map<string, CellInfo>` lookup from all zones and fixtures before render. Each cell does an O(1) map lookup (using the `ck(row, col)` key function) instead of iterating over all zones/fixtures. This keeps rendering fast even for large grids.

### Component Architecture

```
StoreLayoutPage (orchestrator)
├── PageHeader
├── VersionSelector          ← Layout version dropdown + activate
├── CreateLayoutForm         ← New version form (empty-state or panel variant)
├── ModeToolbar              ← Zone / Fixture placement toggle
├── FreeformBar / EditBanner ← Context-aware action bars
├── LayoutGrid               ← Interactive CSS grid (memoized)
├── LayoutObjectsPanel       ← Sidebar listing zones + fixtures (memoized)
├── ZoneNameModal            ← Create zone (name + color)
├── FixtureNameModal         ← Create fixture (name + type)
├── ZoneDetailModal          ← View/edit/delete zone
└── FixtureDetailModal       ← View/edit/delete fixture
```

### Shared CSS

Instead of duplicating modal and action-bar styles across 6 components, the feature uses two shared stylesheets:

- **`layout-modal.css`** — All modal styles (overlay, dialog, header, form fields, color/type pickers, action buttons, delete confirmation). Used by ZoneNameModal, FixtureNameModal, ZoneDetailModal, FixtureDetailModal.
- **`action-bar.css`** — Action bar styles (sticky banner, text, buttons). Used by FreeformBar and EditBanner.

### Constants

| File | What It Defines |
| ---- | --------------- |
| `colors.ts` | 8 zone colors (Blue, Green, Amber, Red, Purple, Cyan, Pink, Indigo) — each with `hex` and light `bg` variant |
| `fixtures.ts` | 8 fixture types (Wall, Checkout, Front Desk, Door, Pillar, Restroom, Storage, Stairs) — each with emoji `icon`, accent `hex`, and light `bg` |

### API Endpoints Used

| Operation | Method | Endpoint |
| --------- | ------ | -------- |
| List layouts | `GET` | `/api/stores/{storeId}/layouts` |
| Create layout | `POST` | `/api/stores/{storeId}/layouts` |
| Activate layout | `POST` | `/api/stores/{storeId}/layouts/{layoutId}/activate` |
| Create zone | `POST` | `/api/stores/{storeId}/layouts/{layoutId}/zones` |
| Update zone | `PUT` | `/api/stores/{storeId}/layouts/{layoutId}/zones/{zoneId}` |
| Delete zone | `DELETE` | `/api/stores/{storeId}/layouts/{layoutId}/zones/{zoneId}` |
| Create fixture | `POST` | `/api/stores/{storeId}/layouts/{layoutId}/fixtures` |
| Update fixture | `PUT` | `/api/stores/{storeId}/layouts/{layoutId}/fixtures/{fixtureId}` |
| Delete fixture | `DELETE` | `/api/stores/{storeId}/layouts/{layoutId}/fixtures/{fixtureId}` |

Zone and fixture data is returned inline with the layout response (nested `zones[]` and `fixtures[]` arrays), so no separate GET endpoints are needed.
